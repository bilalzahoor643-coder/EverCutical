...
function generateInstances(isMobile: boolean): ExoInstance[] {
  ...
  // LAYER 1: NEAR — large, covers ALL edges + corners
  {
    ...
    for (const [x, y, z] of pts) {
      instances.push({
        ...
        scale: isMobile ? 0.8 + rand() * 0.2 : 1.0 + rand() * 0.35,
  ...
  {
    const c = isMobile ? 5 : 6
    const r = isMobile ? 3 : 4
    const pts = scatter(-3, -9, c, r, 1.7, 1.2, 0.8, rand)
    for (const [x, y, z] of pts) {
      instances.push({
        baseX: x, baseY: y, baseZ: z,
        scale: isMobile ? 0.55 + rand() * 0.12 : 0.75 + rand() * 0.25,
  ...
  // LAYER 3: MID — medium, dense fill
  {
    ...
    for (const [x, y, z] of pts) {
      instances.push({
        baseX: x, baseY: y, baseZ: z,
        scale: isMobile ? 0.25 + rand() * 0.07 : 0.32 + rand() * 0.1,
  ...
  {
    const c = isMobile ? 6 : 10
    const r = isMobile ? 3 : 5
    const pts = scatter(-18, -35, c, r, 1.4, 0.7, 0.5, rand)
    for (const [x, y, z] of pts) {
      instances.push({
        baseX: x, baseY: y, baseZ: z,
        scale: isMobile ? 0.14 + rand() * 0.05 : 0.18 + rand() * 0.07,
